...
export class Agent {
  ...
  /** Wall-clock ms until which a human holds the wheel. See tick(). */
  driving?: number;
  ...
  constructor(serialized: SerializedAgent) {
    const { id, lastConversation, lastInviteAttempt, inProgressOperation, driving } = serialized;
  ...
  tick(game: Game, now: number) {
    ...
    // A human is steering this body — stay out of the way.
    //
    // Without this the agent takes the wheel back within about a second: the
    // guard below fires whenever the player isn't in a conversation and isn't
    // mid-activity, and `!player.pathfinding || !recentlyAttemptedInvite` is
    // still true while pathfinding is set (there's usually no recent invite
    // attempt). It would issue its own destination and the person's input
    // would look like it did nothing.
    //
    // The lease EXPIRES rather than being cleared by the client, so a closed
    // tab or a dropped connection can't strand the twin under someone's
    // control forever. Free will resumes on its own.
    if (this.driving && now < this.driving) {
      return;
    }
