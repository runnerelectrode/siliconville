import { assertLLMAllowed } from '../gatherville/killswitch';
...
/**
 * Fit a vector to EMBEDDING_DIMENSION.
 *
 * Models trained with Matryoshka Representation Learning (Qwen3-Embedding,
 * OpenAI text-embedding-3, Nomic v1.5) pack coarse-to-fine information front to
 * back, so a prefix slice is a valid lower-dimensional embedding — after
 * re-normalizing, which is NOT optional: truncation breaks unit norm, and our
 * ranking is cosine similarity.
 *
 * This is what makes Qwen3-Embedding-8B usable on a 1024-dim index: full model
 * quality, existing schema, no index rebuild.
 *
 * ⚠️ Truncating a NON-MRL model produces vectors that are quietly meaningless —
 * still unit-length, still cosine-comparable, just uninformative. There is no
 * way to detect this at runtime. Verify the model card says MRL before setting
 * a model whose native width exceeds EMBEDDING_DIMENSION.
 */
export function fitDimension(vector: number[]): number[] {
  if (vector.length === EMBEDDING_DIMENSION) return vector;

  if (vector.length < EMBEDDING_DIMENSION) {
    throw new Error(
      `Embedding too small: got ${vector.length}, index expects ${EMBEDDING_DIMENSION}. ` +
        `Padding would corrupt the vector space — pick a model with at least ` +
        `${EMBEDDING_DIMENSION} dimensions, or change EMBEDDING_DIMENSION (requires reindexing).`,
    );
  }

  const truncated = vector.slice(0, EMBEDDING_DIMENSION);
  const norm = Math.sqrt(truncated.reduce((acc, x) => acc + x * x, 0));
...
export async function chatCompletion(
  ...
  assertLLMAllowed('aitown/chatCompletion');
...
export async function fetchEmbeddingBatch(texts: string[]) {
  assertLLMAllowed('aitown/fetchEmbeddingBatch');
  const config = getLLMConfig();
  ...
  // Fit to the index width before anything downstream tries to store these.
  //
  // EMBEDDING_DIMENSION is 1024 — Ollama's width, and what both vector indexes
  // were built against. A hosted model does not have to agree: OpenRouter's
  // text-embedding-3-small returns 1536, and inserting that into a 1024-wide
  // index fails at write time, far from the cause.
  //
  // fitDimension is gatherville's rule, reused rather than reimplemented, so
  // the two embedding paths cannot drift into truncating differently and
  // producing vectors that are cosine-incomparable with each other. Read its
  // docstring before pointing this at a new model: truncation is only valid
  // for MRL-trained models, and a non-MRL model truncates to something that
  // still looks like a healthy unit vector while meaning nothing.
  return {
    ollama: false as const,
    embeddings: allembeddings.map(({ embedding }) => fitDimension(embedding)),
...
export async function fetchModeration(content: string) {
  assertLLMAllowed('aitown/fetchModeration');
