...
// GATHERVILLE: memory schema now comes from the unified stream, not ./schema.
import { memoryFields } from '../gatherville/schema';
import { IMPORTANCE_MAX, REFLECTION_IMPORTANCE_THRESHOLD } from '../gatherville/constants';
...
export async function rememberConversation(
  ...
  await ctx.runMutation(selfInternal.insertMemory, {
    ...
    // GATHERVILLE: this conversation happened to the twin in-world, not to the
    // human it represents. Required by the unified schema — see provenance in
    // convex/gatherville/schema.ts.
...
async function calculateImportance(description: string) {
  const { content: importanceRaw } = await chatCompletion({
    messages: [
      {
        role: 'user',
        // GATHERVILLE: rescaled 0-9 -> 0-100 to match genagents'
        // `importance_score/singular_v1.txt`. The two scales MUST NOT coexist:
        // every 0-9 score sorts below every 0-100 score, so ai-town-derived
        // memories would silently never be retrieved. docs/GENAGENTS-ANALYSIS.md §2.1.
        content: `On the scale of 0 to 100, where 0 is purely mundane (e.g., brushing teeth, making bed) and 100 is extremely poignant (e.g., a break up, college acceptance), rate the likely poignancy of the following piece of memory.
      Memory: ${description}
      Answer on a scale of 0 to 100. Respond with number only, e.g. "50"`,
      },
    ],
    temperature: 0.0,
    // GATHERVILLE: was 1 — enough for a single digit, not for a 2-3 digit score.
    max_tokens: 4,
  });
  ...
  return Math.max(0, Math.min(IMPORTANCE_MAX, importance));
}
...
export const insertMemory = internalMutation({
  ...
  handler: async (ctx, { agentId: _, embedding, ...memory }): Promise<void> => {
    ...
    // Provenance is supplied by the caller (required by the schema) rather than
    // defaulted here — a silent default is exactly how a self-reported memory
    // would eventually get mislabelled as simulated, or vice versa.
  ...
  handler: async (ctx, { playerId, reflections }) => {
    ...
    for (const { embedding, relatedMemoryIds, ...rest } of reflections) {
      ...
      await ctx.db.insert('memories', {
        ...
        lastAccess,
        ...rest,
        provenance: 'reflection', // GATHERVILLE
        data: {
          type: 'reflection',
          relatedMemoryIds,
        },
...
async function reflectOnMemories(
  ...
  // GATHERVILLE: was 500 on the 0-9 scale; rescaled with importance to 0-100.
